...
#include <LLNF/Utils/histogram.h>
...
int main (int argc, char *argv[]) {
  ...
  for (int i = 0; i != 100; i++){
    ...
    for (int i = 0; i != opt.Batch; i++)
      fprintf(file, "%lf %lf\n", *samples.index({i,1}).data_ptr<double>(), *samples.index({i,2}).data_ptr<double>());
  }
  ...
  auto prob = torch::cat({torch::tensor({{opt.TEval}}, torch::kF64).expand({opt.Dsize * opt.Dsize, 1}), grid[0].flatten().unsqueeze(-1), grid[1].flatten().unsqueeze(-1), torch::tensor({opt.TEval - 0.1}).expand({opt.Dsize * opt.Dsize, opt.MOrder - 2})}, 1); 
